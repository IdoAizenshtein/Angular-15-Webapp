...
@Component({
    ...
    templateUrl: './ccard-editor.component.html',
    encapsulation: ViewEncapsulation.None,
    preserveWhitespaces: false
})
export class CcardEditorComponent implements OnChanges {
                ...
                options: [
                    ...
                    return {
                        label: this.translateService.instant(
                            'autoUpdateTypes-CCARD_TAZRIM.' + val
                        ),
                        value: val
                    };
                })
    ...
    private populateForm(): void {
        if (this.form) {
            ...
            this.form
                ...
                const controls = [this.form.get('total')];
                if (
                    this.mode === EditingType.Single ||
                    (val === 'USER_DEFINED_TOTAL' && this.mode === EditingType.Series)
                ) {
        ...
        if (this.source && this.form) {
            ...
            if (this.form.get('total') !== null) {
                this.form
                    .get('total')
                    .setValue(
                        this.source.total != null ? Math.abs(this.source.total) : null
                    );
            }
                    ...
                    switch (fldName) {
                        ...
                        case 'autoUpdateTypeName':
                            this.form
                                .get(fldName)
                                .setValidators(
                                    this.mode === EditingType.Series ? [Validators.required] : []
                                );
                            break;
                        default:
                            this.form.get(fldName).disable();
