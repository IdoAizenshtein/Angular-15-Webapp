...
@Component({
    selector: 'app-card-foreign-credentials',
    templateUrl: './card-credentials.component.html',
    encapsulation: ViewEncapsulation.None,
    preserveWhitespaces: false
})
export class CardCredentialsComponent implements OnInit, OnChanges {
    public readonly credType: string = 'card';
    public readonly credTypeDescParent: string = 'creditCards';
    ...
    rebuildCredentialsView() {
        ...
        if (this.foreignControls) {
            ...
            Object.keys(this.credentials.controls).forEach((key) => {
                if (key !== this.credType && key !== 'linkedAccount') {
                    this.credentials.removeControl(key);
                }
            });
        }
    ...
    public getResults(): any | null {
        ...
        return {
            bankAuto: keyCode !== null ? this.credentials.get(keyCode).value : null,
